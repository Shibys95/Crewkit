// scripts/uploadVader.js
// Requirements: npm install firebase xlsx
// Run: node scripts/uploadVader.js
...
// Path to Excel file
const EXCEL_PATH = path.resolve(__dirname, 'crewkit_vader_final_ready.xlsx');
...
const equipment = rawRows.map((item, index) => {
  const nameKey = item.equipment_name ?? item['EquipmentName'] ?? item['equipment_name '];
  const rawPrice = item.price_per_day ?? item['PricePerDay'];
  return {
